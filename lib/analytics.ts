...
const EVENT_PARAMETER_ALLOWLIST: Record<DecisionFunnelEventName, readonly AnalyticsParameterKey[]> = {
  ...
  candidate_detail_view: ['store_id', 'source'],
...
export function sanitizeDecisionAnalyticsPayload(
  ...
  for (const [key, value] of Object.entries(parameters)) {
    if (!allowedKeys.has(key as AnalyticsParameterKey) || value === null || value === undefined) {
      continue;
    }

    if (key === 'candidate_source' && (typeof value !== 'string' || !CANDIDATE_SOURCES.has(value))) {
      continue;
    }
