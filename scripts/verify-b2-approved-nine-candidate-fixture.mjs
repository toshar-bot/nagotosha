...
try {
  ...
  for (const eventName of ['candidate_detail_view', 'compare_view', 'store_decided', 'map_click', 'official_click', 'phone_click']) {
    assert(decisionApp.includes(`trackAnalyticsEvent('${eventName}'`), `${eventName}: GA4 event contract missing`);
  }
  assert(decisionApp.includes('store_id: candidateId'), 'GA4 must use the active formal candidate ID');
